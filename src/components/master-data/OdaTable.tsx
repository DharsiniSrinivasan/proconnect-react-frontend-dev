...
import { ChevronsLeft, ChevronsRight, MapPin } from "lucide-react";
...
const OdaTable = () => {
  ...
  const { rateCards, fetchCards, isLoading, pageSize, setPageSize } =
    rateCardStore();
  ...
  (item) => (item?.oda_service_charge ?? 0) > 0
);

 const totalRecords = filteredRateCards?.length || 0;
...
const paginatedData = filteredRateCards?.slice(
                  ...
                  >
                    ...
                    <TableCell className="font-mono text-sm">
                      <span className="text-amber-400">
                        ₹{item.oda_rate.toFixed(0)}
                      </span>
                    </TableCell>
                    <TableCell className="font-mono text-sm">
                      <span className="text-amber-400">
                        ₹{item.oda_service_charge.toFixed(0)}
                      </span>
                    </TableCell>
                  </TableRow>
                ))
